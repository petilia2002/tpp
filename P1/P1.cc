...
int main(int argc, char *argv[]) {
	...
	double currentTickTime = namedMemoryPtr->timeInfo.current_t_tick_count;
	double tickDuration = namedMemoryPtr->timeInfo.tick_duration_t / 1000000000.;
	...
	while(true) {
		...
		currentTickTime = namedMemoryPtr->timeInfo.current_t_tick_count; // сохраняем в переменную
		tickDuration = namedMemoryPtr->timeInfo.tick_duration_t / 1000000000.; // продолжительность одного тика в секундах
