...
#define END_TIME 107 // время работы приложения Т сек
...
int main(int argc, char *argv[]) {
	...
	// Инициализация разделяемого семафора
	if (sem_init(&namedMemoryPtr->sem, 1, 0) != 0) { // pshared = 1, initial value = 0
	    error("P2: ошибка sem_init()");
	}
	cout << "Р2: sem_init(): OK!" << endl;
	...
	double currentTickTime = namedMemoryPtr->timeInfo.current_dt_tick_count;
	double tickDuration = namedMemoryPtr->timeInfo.tick_duration_dt / 1000000000.;
	sem_wait(&(namedMemoryPtr->sem));
	...
	while(true) {
		if (SignalWaitinfo(&set, NULL) == TICK_SIGNAL_SIGUSR1) {
			...
			currentTickTime = namedMemoryPtr->timeInfo.current_dt_tick_count; // сохраняем в переменную
			tickDuration = namedMemoryPtr->timeInfo.tick_duration_dt / 1000000000.; // продолжительность одного тика в секундах
